...
def Trasfer_Matrix_spinful(*filenames,print_details=False)->np.matrix:
    ...
    ## Add spin degeneracy 
    ## in the old basis we have spin-up and spin down
    ## states appearing consecutevly so one needs to 
    ## multiply the spinless states  l by 2 (spin-degeneracy)
    ## and add 2*l+1 to account for the other spin orientation

    spin_deg=2
    list_of_pos_spinful=[] 
    for els in list_of_pos:
        list_of_pos_spinful.append(spin_deg*els)
        list_of_pos_spinful.append(spin_deg*els+1)

    
    T_mat=np.zeros((spin_deg*len(orb_list),spin_deg*len(orb_list)))
    for old,new in enumerate(list_of_pos_spinful):
        T_mat[old,new]=1
